...
import { productsApi, backofficeApi } from '../utils/api';
...
const ProductReviews = ({ productId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await productsApi.createReview(productId, newReview);
  ...
  return (
    <div className="border-t border-slate-200 pt-6 mt-6">
      ...
      {/* Review Form */}
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 p-4 bg-slate-50 rounded-xl" data-testid="review-form">
          <div className="mb-4">
      ...
      {reviews.length === 0 ? (
        ...
        <div className="space-y-4 max-h-64 overflow-y-auto">
          {reviews.map((review) => (
            <div key={review.id} className="p-4 bg-white border border-slate-100 rounded-xl" data-testid="review-item">
